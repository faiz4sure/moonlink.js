...
export class Player {
  ...
  constructor(manager: Manager, node: Node, config: IPlayerConfig) {
    ...
    this.volume =
      config.volume ?? this.manager.options.defaultPlayer?.volume ?? 100;
    this.set(
      "selfDeaf",
      config.selfDeaf ?? this.manager.options.defaultPlayer?.selfDeaf ?? true,
    );
    this.set(
      "selfMute",
      config.selfMute ?? this.manager.options.defaultPlayer?.selfMute ?? false,
    );
    this.autoPlay =
      config.autoPlay ?? this.manager.options.defaultPlayer?.autoPlay ?? false;
    this.autoLeave =
      config.autoLeave ??
      this.manager.options.defaultPlayer?.autoLeave ??
      false;
    this.loop =
      config.loop ?? this.manager.options.defaultPlayer?.loop ?? "off";
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#constructor >> Player created for guild ${this.guildId} on node ${this.node.identifier} | autoPlay: ${this.autoPlay}, autoLeave: ${this.autoLeave}, loop: ${this.loop}`,
    );
  ...
  private async performHealthCheck(): Promise<void> {
    ...
    const now = Date.now();
    const idleTimeout =
      this.manager.options.playerDestruction?.idleTimeout ?? 300000;
    const silenceTimeout =
      this.manager.options.playerHealth?.silenceTimeout ?? 120000;
    const maxStuckCount = this.manager.options.playerHealth?.maxStuckCount ?? 3;
    const maxSilentCount =
      this.manager.options.playerHealth?.maxSilentCount ?? 2;
    ...
    if (this.playing && !this.paused && this.current) {
      ...
      if (positionDelta < 1000 && timeDelta > 30000 && !this.current.isStream) {
        ...
        this.manager.emit(
          "debug",
          `Moonlink.js > Player#healthCheck >> Player ${this.guildId} appears stuck. Position: ${currentPosition}, Stuck count: ${this.stuckDetectionCount}/${maxStuckCount}`,
        );

        if (this.stuckDetectionCount >= maxStuckCount) {
          this.manager.emit(
            "debug",
            `Moonlink.js > Player#healthCheck >> Player ${this.guildId} stuck for too long, attempting recovery.`,
          );
      ...
      const trackStartedAt = this.get<number>("trackStartedAt") ?? now;
      const playingDuration = now - trackStartedAt;

      if (activityDelta > silenceTimeout && playingDuration > silenceTimeout) {
        this.silentDetectionCount++;
        this.manager.emit(
          "debug",
          `Moonlink.js > Player#healthCheck >> Player ${this.guildId} silent for ${Math.round(activityDelta / 1000)}s (playing for ${Math.round(playingDuration / 1000)}s). Silent count: ${this.silentDetectionCount}/${maxSilentCount}`,
        );

        if (this.silentDetectionCount >= maxSilentCount) {
          this.manager.emit(
            "debug",
            `Moonlink.js > Player#healthCheck >> Player ${this.guildId} silent for too long, attempting soft restart.`,
          );
        ...
        if (this.silentDetectionCount > 0) {
          this.manager.emit(
            "debug",
            `Moonlink.js > Player#healthCheck >> Player ${this.guildId} silence counter reset (activityDelta=${Math.round(activityDelta / 1000)}s, playingDuration=${Math.round(playingDuration / 1000)}s).`,
          );
        }
        this.silentDetectionCount = 0;
      }
  ...
  private async recoverFromStuck(): Promise<void> {
    ...
    try {
      if (this.current && this.current.isSeekable) {
        ...
        this.manager.emit(
          "debug",
          `Moonlink.js > Player#recoverFromStuck >> Seeking forward 2s for player ${this.guildId}.`,
        );
      ...
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#recoverFromStuck >> Recovery failed: ${(error as Error).message}`,
      );
  ...
  public async softRestart(reason: string): Promise<boolean> {
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#softRestart >> Soft restart triggered for player ${this.guildId}. Reason: ${reason}`,
    );
    ...
    if (!currentTrack) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#softRestart >> No current track, skipping soft restart for player ${this.guildId}.`,
      );
      return false;
    }
    ...
    try {
      ...
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#softRestart >> Soft restart completed for player ${this.guildId}.`,
      );
      return true;
    } catch (error) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#softRestart >> Soft restart failed: ${(error as Error).message}`,
      );
      return false;
    }
  }
  ...
  private resolveEncodedTrack(track: Track | string): string {
    ...
    throw new Error(
      "Invalid track input. Provide a Track instance or encoded string.",
    );
  }

  private async sendPlayerUpdate(
    ...
    if (payload.voice) {
      ...
      if (
        this.node.isNodeLink &&
        (!voicePayload.sessionId ||
          !voicePayload.token ||
          !voicePayload.endpoint)
      ) {
        throw new Error(
          "voice payload requires sessionId, token, and endpoint.",
        );
      }
  ...
  public async connect({
    selfDeaf,
    selfMute,
  }: { selfDeaf?: boolean; selfMute?: boolean } = {}): Promise<this> {
    const connectStack = new Error().stack?.split("\n").slice(2, 8).join("\n");
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#connect -> Connect requested for guild ${this.guildId}. Stack: ${connectStack || "unavailable"}`,
    );
  ...
  public async disconnect(): Promise<this> {
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#disconnect -> Disconnect requested for guild ${this.guildId}. Stack: ${disconnectStack || "unavailable"}`,
    );
  ...
  public async play(
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#play -> play() called for guild ${this.guildId} with options: ${JSON.stringify(options)}`,
    );

    if (!this.voice.sessionId || !this.voice.endpoint) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#play >> Voice not ready for guild ${this.guildId}, attempting to connect...`,
      );
      try {
        await this.connect();
      } catch (e) {
        this.manager.emit(
          "debug",
          `Moonlink.js > Player#play >> Failed to connect voice: ${e}`,
        );
        return false;
      }
    } else {
    ...
    if (finalOptions.encoded) {
      try {
        ...
        this.manager.emit(
          "debug",
          `Moonlink.js > Player#play >> Error decoding track for guild ${this.guildId}. Error: ${e}`,
        );
        return false;
      }
    }
    ...
    if (track && !(track instanceof Track)) {
      try {
        ...
        this.manager.emit(
          "debug",
          `Moonlink.js > Player#play >> Invalid track object for guild ${this.guildId}. Error: ${e}`,
        );
        return false;
      }
    }
    ...
    if (track) {
      ...
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#play >> Added track "${track.title}" to front of queue for guild ${this.guildId}`,
      );
    }
    ...
    if (this.queue.size === 0) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#play >> Queue is empty, cannot start playback for guild ${this.guildId}`,
      );
      return false;
    }
    ...
    if (!nextTrack || !nextTrack.encoded) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#play >> Invalid track data for guild ${this.guildId}. Next track is ${JSON.stringify(nextTrack)}`,
      );
    ...
    this.current =
      nextTrack instanceof Track ? nextTrack : new Track(nextTrack);
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#play >> Player state changed: current track: ${oldTrackTitle} -> ${this.current.title}`,
    );
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#play -> Sending play request to node ${this.node.identifier} for guild ${this.guildId}. Payload: ${JSON.stringify(payload)}`,
    );

    try {
      await this.sendPlayerUpdate(
        payload,
        finalOptions.noReplace ?? this.manager.options.noReplace,
      );
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#play >> Successfully sent play request for track "${this.current.title}" for guild ${this.guildId}`,
      );
    } catch (e) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#play >> CRITICAL: Failed to send play request for guild ${this.guildId}. Reverting state. Error: ${(e as Error).message}`,
      );
    ...
    if (previousTrack && !this.get("isBackPlay")) {
      ...
      const historyLimit =
        this.manager.options.queue?.historyLimit ?? this.historySize;
      ...
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#play << Added track "${previousTrack.title}" to history for guild ${this.guildId}. History size: ${this.previous.length}`,
      );
    }
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#play >> Player state changed: playing: ${oldPlaying} -> ${this.playing}, paused: ${oldPaused} -> ${this.paused}`,
    );
  ...
  public async addMix(
    track: Track | string,
    options?: { volume?: number; userData?: Record<string, any> },
  ): Promise<any | null> {
  ...
  /** NodeLink-only feature. See https://github.com/PerformanC/NodeLink */
  public async subscribeLyrics(options?: {
    skipTrackSource?: boolean;
  }): Promise<this> {
    this.assertNodeLinkFeature("lyrics:subscribe");
    await this.node.rest.subscribeLyrics(
      this.guildId,
      options?.skipTrackSource,
    );
    return this;
  }
  ...
  public async loadChapters(
    encodedTrack?: string,
  ): Promise<NodeLinkResponse<IChapter[]> | null> {
    this.assertNodeLinkFeature("chapters");
    const target = encodedTrack ?? this.current?.encoded;
    if (!target) {
      throw new Error(
        "loadChapters requires an encoded track or an active player track.",
      );
    }
  ...
  /** NodeLink-only feature. See https://github.com/PerformanC/NodeLink */
  public async loadMeaning(
    encodedTrack?: string,
    lang?: string,
  ): Promise<NodeLinkResponse<IMeaningResponse | Record<string, any>> | null> {
    this.assertNodeLinkFeature("meaning");
    const target = encodedTrack ?? this.current?.encoded;
    if (!target) {
      throw new Error(
        "loadMeaning requires an encoded track or an active player track.",
      );
    }
  ...
  /** NodeLink-only feature. See https://github.com/PerformanC/NodeLink */
  public async getLoadDirectStream(options?: {
    volume?: number;
    position?: number;
    ...
    return this.node.loadDirectStream(
      target,
      options?.volume,
      options?.position,
      options?.filters,
  ...
  public async pause(): Promise<this> {
    if (this.paused) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#pause >> Player is already paused for guild ${this.guildId}`,
      );
      return this;
    }
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#pause -> Sending pause request to node ${this.node.identifier} for guild ${this.guildId}`,
    );
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#pause >> Player state changed: paused: ${oldPaused} -> ${this.paused}`,
    );

    this.manager.emit(
      "debug",
      `Moonlink.js > Player#pause >> Player paused for guild ${this.guildId}`,
    );
    return this;
  }
  ...
  public async resume(options?: { timeout?: number }): Promise<this> {
    const now = Date.now();
    const minResumeInterval =
      this.manager.options.playerHealth?.resumeDebounce ?? 1000;

    if (this.isResumeInProgress) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#resume >> Resume already in progress for guild ${this.guildId}, skipping.`,
      );
      return this;
    }
    ...
    if (now - this.lastResumeAttempt < minResumeInterval) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#resume >> Resume debounced for guild ${this.guildId} (last attempt ${now - this.lastResumeAttempt}ms ago).`,
      );
      return this;
    }
    ...
    if (!this.paused) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#resume >> Player is not paused for guild ${this.guildId}`,
      );
      return this;
    }
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#resume -> Sending resume request to node ${this.node.identifier} for guild ${this.guildId}`,
    );
    ...
    try {
      ...
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#resume >> Player state changed: paused: ${oldPaused} -> ${this.paused}`,
      );

      this.manager.emit(
        "debug",
        `Moonlink.js > Player#resume >> Player resumed for guild ${this.guildId}`,
      );
    } finally {
  ...
  public async setVoiceState(state: {
    channelId?: string;
    selfMute?: boolean;
    selfDeaf?: boolean;
  }): Promise<this> {
  ...
  public async stop(): Promise<this> {
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#stop -> Sending stop request to node ${this.node.identifier} for guild ${this.guildId}`,
    );
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#stop >> Player state changed: playing: ${oldPlaying} -> ${this.playing}`,
    );
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#stop >> Player state changed: current: ${oldTrackTitle} -> null`,
    );

    this.manager.emit(
      "debug",
      `Moonlink.js > Player#stop >> Player stopped for guild ${this.guildId}`,
    );
    return this;
  }
  ...
  public async skip(position?: number): Promise<boolean> {
    ...
    if (position !== undefined) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#skip -> Skipping to position ${position} in queue for guild ${this.guildId}`,
      );
      const track = this.queue.remove(position);
      if (!track) {
        this.manager.emit(
          "debug",
          `Moonlink.js > Player#skip >> Invalid queue position for guild ${this.guildId}`,
        );
        return false;
      }
      this.manager.emit("playerTriggeredSkip", this, oldTrack, track, position);
      return await this.play({
        track: track instanceof Track ? track : new Track(track),
      });
    }
    ...
    if (!this.queue.size) {
      if (this.autoPlay) {
        if (!this.current) {
          this.manager.emit(
            "debug",
            `Moonlink.js > Player#skip >> Queue is empty and no current track, cannot trigger autoPlay for guild ${this.guildId}`,
          );
          return false;
        }
        this.manager.emit(
          "debug",
          `Moonlink.js > Player#skip >> Queue empty, triggering autoPlay for guild ${this.guildId}`,
        );
        return await this.node.handleAutoPlay(this, this.current);
      }
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#skip >> Queue is empty, stopping player for guild ${this.guildId}`,
      );
      await this.stop();
      return true;
    }
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#skip -> Skipping to next track in queue for guild ${this.guildId}`,
    );
    return await this.play();
  }
  ...
  public async seek(position: number): Promise<this> {
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#seek -> Seeking to position ${position}ms for guild ${this.guildId}`,
    );
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#seek >> Sent seek request to node ${this.node.identifier}`,
    );
  ...
  public setVolume(volume: number): this {
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#setVolume -> Changing volume from ${oldVolume} to ${volume} for guild ${this.guildId}`,
    );
    this.sendPlayerUpdate({ volume });
    this.updateData("volume", this.volume);
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#setVolume >> Volume updated for guild ${this.guildId}`,
    );
  ...
  public setLoop(loop: PlayerLoop, count?: number): this {
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#setLoop >> Loop mode changed from "${oldLoop}" to "${loop}"${count ? ` (count: ${count})` : ""} for guild ${this.guildId}`,
    );
    return this;
  }
  ...
  public setAutoPlay(autoPlay: boolean): this {
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#setAutoPlay >> AutoPlay set to ${autoPlay} for guild ${this.guildId}`,
    );
    return this;
  }
  ...
  public setAutoLeave(autoLeave: boolean): this {
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#setAutoLeave >> AutoLeave set to ${autoLeave} for guild ${this.guildId}`,
    );
    return this;
  }
  ...
  public shuffle(): this {
    if (this.queue.size < 2) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#shuffle >> Queue has less than 2 tracks, cannot shuffle for guild ${this.guildId}`,
      );
      return this;
    }
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#shuffle -> Shuffling queue with ${this.queue.size} tracks for guild ${this.guildId}`,
    );
    this.queue.shuffle();
    this.manager.emit(
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#shuffle >> Queue shuffled for guild ${this.guildId}`,
    );
    return this;
  }
  ...
  public async destroy(reason?: string): Promise<void> {
    ...
    if (!reason) {
      ...
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#destroy >> Destroy called without reason for guild ${this.guildId}. Stack: ${stack || "unavailable"}`,
      );
    }
    const destroyReason = reason ?? "No reason provided";
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#destroy -> Destroying player for guild ${this.guildId}. Reason: ${destroyReason}`,
    );
    ...
    try {
      ...
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#destroy >> Voice disconnection failed for guild ${this.guildId}: ${(e as Error).message}`,
      );
    }
    try {
      ...
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#destroy >> Failed to destroy player on node: ${(e as Error).message}`,
      );
    }
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#destroy >> Player destroyed for guild ${this.guildId}.`,
    );
  }
  ...
  public async restart(): Promise<boolean> {
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#restart -> Restarting player for guild ${this.guildId}`,
    );
    ...
    if (!this.voiceChannelId) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#restart >> Cannot restart player without a voiceChannelId for guild ${this.guildId}`,
      );
      return false;
    }
    ...
    try {
      ...
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#restart >> Voice connection failed for guild ${this.guildId}. Error: ${(error as Error).message}`,
      );
    ...
    const voicePayload =
      this.voice.sessionId && this.voice.token && this.voice.endpoint
        ? {
            sessionId: this.voice.sessionId,
            token: this.voice.token,
            endpoint: this.voice.endpoint,
    ...
    if (voicePayload) {
      ...
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#restart >> Voice data missing for guild ${this.guildId}, skipping voice refresh.`,
      );
    }
    ...
    if (this.current) {
      ...
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#restart -> Restoring current track "${this.current.title}" for guild ${this.guildId} at ${resumePosition}ms.`,
      );
      this.playing = true;
      this.paused = false;
      ...
      if (resumePosition > 0 && this.current.isSeekable && voicePayload) {
        ...
        this.manager.emit(
          "debug",
          `Moonlink.js > Player#restart >> Seek skipped (position: ${resumePosition}ms, seekable: ${this.current.isSeekable}, voiceReady: ${Boolean(voicePayload)}) for guild ${this.guildId}.`,
        );
      }
    } else if (this.queue.size > 0) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#restart -> No current track, playing first from queue for guild ${this.guildId}`,
      );
      await this.play();
    } else {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#restart >> No track to restore and queue is empty for guild ${this.guildId}`,
      );
      return true;
    }
  ...
  public async transferNode(node: Node | string): Promise<boolean> {
    const targetNode =
      typeof node === "string" ? this.manager.nodes.get(node) : node;

    if (
      !targetNode ||
      !targetNode.connected ||
      targetNode.identifier === this.node.identifier
    ) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#transferNode >> Invalid target node for guild ${this.guildId}`,
      );
      return false;
    }
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#transferNode -> Transferring player ${this.guildId} from ${oldNode.identifier} to ${targetNode.identifier}`,
    );
    ...
    try {
      ...
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#transferNode >> Failed to destroy player on old node: ${e.message}`,
      );
    }
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#transferNode >> Successfully transferred player ${this.guildId} to ${targetNode.identifier}`,
    );
    return true;
  }
  ...
  public setVoiceChannelId(voiceChannelId: string): this {
    ...
    if (this.voiceChannelId === voiceChannelId) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#setVoiceChannelId >> Voice channel already set to ${voiceChannelId} for guild ${this.guildId}`,
      );
      return this;
    }
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#setVoiceChannel >> Voice channel changed from ${oldChannel} to ${voiceChannelId} for guild ${this.guildId}`,
    );
    this.manager.emit(
  ...
  public setTextChannelId(textChannelId: string): this {
    ...
    if (this.textChannelId === textChannelId) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#setTextChannelId >> Text channel already set to ${textChannelId} for guild ${this.guildId}`,
      );
      return this;
    }
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#setTextChannel >> Text channel changed from ${oldChannel} to ${textChannelId} for guild ${this.guildId}`,
    );
    this.manager.emit(
  ...
  public async replay(): Promise<boolean> {
    if (!this.current?.encoded) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#replay >> No current track to replay for guild ${this.guildId}`,
      );
      return false;
    }
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#replay -> Replaying track "${this.current.title}" for guild ${this.guildId}`,
    );
  ...
  public async back(): Promise<boolean> {
    if (this.previous.length === 0) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#back >> No previous tracks available for guild ${this.guildId}`,
      );
      return false;
    }
    ...
    if (!lastTrack) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#back >> Popped an invalid track from history for guild ${this.guildId}`,
      );
      return false;
    }
    ...
    this.manager.emit(
      "debug",
      `Moonlink.js > Player#back -> Going back to previous track "${lastTrack.title}" for guild ${this.guildId}`,
    );
    ...
    if (this.current) {
      ...
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#back >> Pushed current track "${this.current.title}" to the front of the queue.`,
      );
    }
    ...
    if (played) {
      ...
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#back >> Successfully went back for guild ${this.guildId}`,
      );
    } else {
      this.manager.emit(
        "debug",
        `Moonlink.js > Player#back >> Failed to play previous track for guild ${this.guildId}.`,
      );
    }
