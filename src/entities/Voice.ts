...
export class Voice extends EventEmitter<VoiceEvents> {
  ...
  public connect(options: {
    ...
    if (!this.player.voiceChannelId) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Voice#connect >> Missing voiceChannelId for guild ${this.player.guildId}, skipping connect.`,
      );
    ...
    if (this.state === VoiceConnectionState.CONNECTED) {
      this.manager.emit(
        "debug",
        `Moonlink.js > Voice#connect >> Connected state without voice data for guild ${this.player.guildId}, forcing reconnect.`,
      );
      this.setState(VoiceConnectionState.DISCONNECTED);
    }
      ...
      this.connectPromise = new Promise((resolve, reject) => {
        ...
        this.once("disconnect", (err?: Error) => {
          ...
          reject(
            err ||
              new Error(
                "Connection was disconnected during connection attempt.",
              ),
          );
        });
      });
  ...
  public async handleStateUpdate(data: VoiceStateUpdate): Promise<void> {
    ...
    if (
      ...
      this.manager.emit(
        "debug",
        `Moonlink.js > Voice#handleStateUpdate >> Bot moved from ${oldChannelId} to ${data.channel_id}. Preserving playback for guild ${this.player.guildId}.`,
      );
  ...
  public check(connected: boolean): void {
    if (connected) {
      if (!this.lastConnectionStatus) {
        this.manager.emit(
          "debug",
          `Player ${this.player.guildId} reconnected. Clearing recovery timer.`,
        );
    ...
    if (!userInitiated) {
      ...
      this.manager.emit(
        "debug",
        `Player ${this.player.guildId} recovery skipped: connection not user-initiated.`,
      );
      return;
    }
    ...
    if (this.lastConnectionStatus) {
      this.manager.emit(
        "debug",
        `Player ${this.player.guildId} connection lost. Scheduling 5s recovery timer (playing=${this.player.playing}).`,
      );
      this.lastConnectionStatus = false;
      ...
      this.reconnectionTimer = setTimeout(() => {
        ...
        if (!this.player.connected && !this.player.destroyed) {
          this.manager.emit(
            "debug",
            `Player ${this.player.guildId} still disconnected after 5s. Attempting recovery.`,
          );
          this.recover();
        }
      }, 5000);
  ...
  private async recover(): Promise<void> {
    ...
    try {
      this.manager.emit(
        "debug",
        `Player ${this.player.guildId} recovery: Attempting soft reconnect.`,
      );
      const store = {
      ...
      this.manager.emit(
        "debug",
        `Player ${this.player.guildId} recovery: Soft reconnect failed. Attempting hard restart. Error: ${(softError as Error).message}`,
      );
      try {
        this.isMoving = true;
        ...
        this.manager.emit(
          "debug",
          `Player ${this.player.guildId} recovery: Hard restart failed. Destroying player. Error: ${(hardError as Error).message}`,
        );
  ...
  private checkCompletion(): void {
    if (this.sessionId && this.token && this.endpoint) {
      ...
      if (forceResend && samePayload) {
        this.manager.emit(
          "debug",
          `Moonlink.js > Voice#checkCompletion >> Force-resending voice update for guild ${this.player.guildId} (${Math.round(timeSinceLastSend / 60000)}min since last send).`,
        );
      }
      ...
      this.player
        ...
        .then(async () => {
          ...
          if (
            ...
            try {
              this.manager.emit(
                "debug",
                `Moonlink.js > Voice#checkCompletion >> Restoring playback after channel move for guild ${this.player.guildId}.`,
              );
              const restarted = await this.player.restart();
              if (!restarted) {
                this.manager.emit(
                  "debug",
                  `Moonlink.js > Voice#checkCompletion >> Restart returned false after channel move for guild ${this.player.guildId}.`,
                );
              }
            } catch (restartError) {
              this.manager.emit(
                "debug",
                `Moonlink.js > Voice#checkCompletion >> Restart failed after channel move for guild ${this.player.guildId}. Error: ${(restartError as Error).message}`,
              );
            } finally {
        ...
        .catch((e) => {
          ...
          this.manager.emit(
            "debug",
            `Failed to send voice update: ${e.message}`,
          );
          this.pendingPlaybackRestoreNonce = null;
          this.isMoving = false;
